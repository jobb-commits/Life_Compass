export default async function handler(req, res) {
    ...
    // Get API key from Vercel's Environment Variables 
    ...
    try {
        // Then securely call Gemini API from the server
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: {
                    response_mime_type: "application/json", // Force output to be in JSON format
                    maxOutputTokens: 8192, // Headroom for 5 resources + 3 insights even with a long goals list
                    thinkingConfig: {
                        thinkingBudget: 1024 // Cap internal reasoning so it doesn't eat whole token budget
                    }
                }
            })
        });
        ...
        // Flag truncated responses so the frontend can handle them gracefully
